//! This example demonstrates eagerly loading and memoized lazy parsing descriptions from pacman packages that have been installed into the current system.
//!
//! **What it does:**
//! * Eagerly loading all desc files from `/var/lib/pacman/local/` with [`arch_pkg_db::TextCollection::par_from_local_db`].
//! * Loading desc texts into [`arch_pkg_db::desc::MemoQuerier`] objects without parsing using [`arch_pkg_db::TextCollection::par_parse_mut`].
//! * Asking for a package by name, looking it up using [`arch_pkg_db::QueryDatabase::get_mut`].
//! * Displaying some information of the queried package using methods in [`arch_pkg_db::desc::QueryMut`] (which trigger on-demand parsing).
...
fn main() -> ExitCode {
    ...
    loop {
        ...
        println!("Package Name: {}", pkg.name_mut().unwrap_or(Name("-")));
        println!("Package Base: {}", pkg.base_mut().unwrap_or(Base("-")));
        println!("Version: {}", pkg.version_mut().unwrap_or(Version("-")));
        println!(
            "Description: {}",
            pkg.description_mut().unwrap_or(Description("-")),
        );
        println!("URL: {}", pkg.url_mut().unwrap_or(Url("-")));

        let provides = pkg.provides_mut();
