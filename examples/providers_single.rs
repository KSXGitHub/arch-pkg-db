//! This example demonstrates eagerly loading and parsing descriptions from pacman's merged sync databases (repository packages) and querying them
//! by `provides`.
//!
//! **What it does:**
//! * Eagerly loading desc files from repository databases (`core.db`, `extra.db`, `multilib.db`) in `/var/lib/pacman/sync/` using
//!   [`arch_pkg_db::TextCollection::extend_from_archive`] to extract archives (with automatic MIME type detection) into a repository-unaware
...
use arch_pkg_db::{
    ...
    value::{Base, Dependency, DependencyList, DependencyName, Description, Name, Url, Version},
};
use pipe_trait::Pipe;
use std::{
    fs::{metadata, read},
